...
namespace EasyCheers.tools
{
    export class neotool
    {
        ...
        public static verifyAddress(addr: string): boolean
        {
            ...
            var res: boolean = verify.test(addr) ? neotool.verifyPublicKey(addr) : verify.test(addr);
            return res;
        }
            ...
            try
            {
                ...
                if (wallet.accounts)
                {
                    for (let keyindex = 0; keyindex < wallet.accounts.length; keyindex++)
                    {
                        ...
                        try
                        {
                            let result: Result = await tools.neotool.getPriKeyfromAccount(wallet.scrypt, password, account);
