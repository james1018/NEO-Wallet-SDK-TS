namespace EasyCheers.tools {
    export class Contract
    {
        ...
        static async buildInvokeTransData_attributes(script: Uint8Array)
        {
            ...
            var gass = utxos[CoinTool.id_GAS];
            ...
            {
                feeres = CoinTool.creatInuptAndOutup(gass, Neo.Fixed8.fromNumber(0.001));
        ...
        static async buildInvokeTransData(...param: any[])
        {
            ...
            let assetid = have ? param[2] : CoinTool.id_GAS;
            let count = have ? param[3] : Neo.Fixed8.Zero;
            //获得utxo,构造交易
            var utxos = await CoinTool.getassets();
            let tranmsg = CoinTool.makeTran(utxos, addr, assetid, count);
        ...
        static async contractInvokeScript(appCall: Neo.Uint160, method: string, ...param: string[])
        {
            ...
            return await WWW.rpc_getInvokescript(data);
        }
        ...
        static async contractInvokeTrans_attributes(script: Uint8Array)
        {
            ...
            let payfee :boolean = false;
            //let payfee :boolean = LoginInfo.info.payfee;
            if (gass && payfee)
            {
                feeres = CoinTool.creatInuptAndOutup(gass, Neo.Fixed8.fromNumber(0.001));
                tran.inputs = feeres.inputs.map(input =>
        ...
        {
            let res = await tools.WWW.getNotify(txid);
