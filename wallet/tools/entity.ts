namespace EasyCheers {
    ...
    export class Process
    {
        ...
        constructor(start: number | string)
        {
            ...
            this.date = tools.DateTool.dateFtt("yyyy/MM/dd", new Date(this.startTime));
            this.time = tools.DateTool.dateFtt("hh:mm:ss", new Date(this.startTime));
            this.width = 0;
            this.state = "";
            for (let i = 1; i <= 5; i++)
            {
                ...
                let date = tools.DateTool.dateFtt("yyyy/MM/dd", new Date(time));
                let times = tools.DateTool.dateFtt("hh:mm:ss", new Date(time));
                element.date = date;
                element.time = times;
